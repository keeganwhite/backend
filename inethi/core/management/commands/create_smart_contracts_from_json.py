...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--default-user',
            type=str,
            help='Default username to assign contracts to if user_name not specified in JSON (defaults to SUPERUSER_USERNAME from env)'
        )
    ...
    def handle(self, *args, **options):
        ...
        # Get default user
        try:
            default_user_obj = User.objects.get(username=default_user)
        ...
        for i, contract_data in enumerate(contracts_data):
            try:
                ...
                if dry_run:
                    self.stdout.write(
                        f'Would create contract: {name} ({contract_type}) at {address} assigned to user: {user_name}'
                    )
                    continue
                ...
                with transaction.atomic():
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f'Created contract "{name}" ({contract_type}) at {address} assigned to user: {user_name}'
                        )
                    )
        ...
        if dry_run:
            ...
            if created_contracts:
                ...
                for contract in created_contracts:
                    self.stdout.write(f'  - {contract["name"]} ({contract["contract_type"]}) at {contract["address"]} assigned to {contract["user"]}')
