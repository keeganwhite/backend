"""
CryptoUtils with manual transaction building for Web3.py v7 compatibility
Works with Celo via https://forno.celo.org
"""
...
class CryptoUtils:
    """
    Utility class for blockchain interactions with manual transaction building.
    ...
    def send_to_wallet_address(
        ...
        """Send tokens to a wallet address with nonce management and retry."""
    ...
    def faucet_balance_threshold(self, address: str) -> float:
        ...
        try:
            ...
            logger.error(f'Error calling nextBalance: {e}')
            return 0.0
    ...
    def balance_of_celo(self, address: str) -> float:
        ...
        try:
            ...
            logger.error(f"Error getting CELO balance: {e}")
            return 0.0

    def check_gas_status(self, from_address: str, gas_amount: int) -> bool:
        """Check if address has enough gas for transaction."""
        try:
            balance = self.w3.eth.get_balance(Web3.to_checksum_address(from_address))
            gas_price = self.w3.eth.gas_price
            required_gas = gas_amount * gas_price
            return balance >= required_gas
        except Exception as e:
            logger.error(f"Error checking gas status: {e}")
            return False
